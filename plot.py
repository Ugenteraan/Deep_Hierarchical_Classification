...
def plot_loss_acc(path, num_epoch, train_accuracies_superclass, train_accuracies_subclass, train_losses,
    ...
    test_subclass_accuracy_df = pd.DataFrame({"Epochs":epochs, "Accuracy":train_accuracies_subclass, "Mode":['train']*(num_epoch+1)})
    ...
    data = pd.concat([train_superclass_accuracy_df, test_subclass_accuracy_df, test_superclass_accuracy_df, test_subclass_accuracy_df])
